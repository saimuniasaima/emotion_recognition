...
def preprocess_image(file):
    try:
        img = skimage.io.imread(file, as_gray = True) # open image into grayscale
        img =  resize(img, dimension, anti_aliasing=True, mode='reflect') #resize the image of 48*48 size
        img.shape = img.shape+tuple([num_channels]) #Change the shape from 2-dim to 3-dim
...
def train_model():
    # train the model
    checkpointer = ModelCheckpoint(filepath='./model/saved_model.h5', 
...
def confussion_matrix():
    ...
    plt.imshow(cm, interpolation='nearest', cmap = plt.cm.Blues) #cmap = plt.cm.Greens
...
#Split data
X_train, X_test, y_train, y_test = train_test_split(images, yy, test_size=0.3,random_state=109)
...
evaluate_model()
confussion_matrix()
print_prediction('test/Aaron_Peirsol_0002.jpg')